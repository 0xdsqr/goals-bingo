...
import { Authenticated, Unauthenticated, useMutation, useQuery, useAction, useConvexAuth } from "convex/react";
...
function HomePage() {
  ...
  const handleRankDifficulty = async () => {
    ...
    const goalsWithText = localBoard.goals.filter(g => g.text && !g.isFreeSpace);
    ...
    <div className="container mx-auto px-4 py-6 max-w-3xl">
      <header className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <Authenticated>
            <Link to="/boards">
              <Button variant="outline" size="sm">My Boards</Button>
            </Link>
          ...
          <Unauthenticated>
            <Button size="sm" onClick={() => setShowSignIn(true)}>Sign In</Button>
          </Unauthenticated>
        </div>
      </header>
        ...
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center justify-between gap-2 text-base">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <span className="text-muted-foreground shrink-0 text-sm">Board:</span>
              {isEditingName ? (
                <Input
          ...
          <div className="mt-4 pt-4 border-t border-border">
            ...
            {difficultyRank && (
              <div className="mt-3 p-3 bg-muted rounded-lg">
                ...
                <p className="text-xs text-muted-foreground whitespace-pre-wrap">{difficultyRank}</p>
              </div>
            )}
            <Unauthenticated>
              ...
              <div className="space-y-2">
                {eventFeed && eventFeed.length > 0 ? (
                  eventFeed
                    ...
                    .map((event) => (
                      <EventFeedItemComponent key={event._id} event={event as EventFeedItem} />
                    ))
                ) : (
